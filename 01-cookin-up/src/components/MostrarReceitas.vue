...
export default {
    ...
    async created() {
        const listaReceitas = await obterReceitas();

        this.receitasEncontradas = listaReceitas.filter(x => this.filtro.every(v => x.ingredientes.includes(v)));
    },
    emits: ['editarLista'],
    components: { BotaoPrincipal, CardReceita },
    props: {
        filtro: { type: [] as PropType<string[]>, required: false, default: [] }
    }
}
</script>
<template>
